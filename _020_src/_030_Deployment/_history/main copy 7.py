...
df_features['ewm'] = df['calls'].ewm(span=2).mean()  # Exponentiell gewichteter Mittelwert
...
features = df_features[['lag_1', 'lag_7', 'lag_30', 'sin_month', 'cos_month', 'sin_dayofweek', 'cos_dayofweek', 
                        'rolling_mean_7', 'rolling_mean_30', 'rolling_std_7', 'trend', 'ewm']].values
...
xgb_model = xgb.XGBRegressor(objective="reg:squarederror", n_estimators=500, max_depth=8, learning_rate=0.025, alpha=0.1, random_state=42)

xgb_model.fit(X_train, y_train)

# Ermittlung von dem Offset für Prediction
y_train_pred = xgb_model.predict(X_train)




max_diff = np.maximum(y_train_pred, y_train)
print(max_diff.mean())

rmse_train = np.sqrt(mean_squared_error(y_train, y_train_pred)) #/ np.mean(y_train) * 100
print(rmse_train)



# Vorhersagen auf den Testdaten
#xgb_pred_test = xgb_model.predict(X_test)#*1.14
...
#xgb_pred_test = np.maximum(xgb_pred_test, df_features.loc[test_indices, 'lag_1'].values) # Verhindere, dass Vorhersagen unter 'lag_1' liegen
...
future_features['lag_1'] = df['calls'].shift(1).iloc[-30:].values
future_features['lag_7'] = df['calls'].shift(7).iloc[-30:].values
future_features['lag_30'] = df['calls'].shift(30).iloc[-30:].values
...
# Gleitende Durchschnitte
future_features['rolling_mean_7'] = df['calls'].rolling(window=7).mean().iloc[-30:].values
future_features['rolling_mean_30'] = df['calls'].rolling(window=30).mean().iloc[-30:].values
future_features['rolling_std_7'] = df['calls'].rolling(window=7).std().iloc[-30:].values
...
future_features['ewm'] = df['calls'].ewm(span=2).mean()  # Exponentiell gewichteter Mittelwert
...
# Korrektur auf die Zukunftsvorhersagen anwenden
#future_predictions = np.maximum(future_predictions, future_features['lag_1'].values) # Verhindere, dass Vorhersagen unter 'lag_1' liegen
