...
df_pred['lag_5'] = df_pred['calls'].rolling(window=3).mean().shift(1)
df_pred['lag_6'] = df_pred['calls'].rolling(window=7).mean().shift(1)
...
predictions = []
actual = []
total_iterations = len(X) - train_size


# Perform walk-forward validation
for i in range(total_iterations):
    X_train, X_test = X[i : i + train_size], X[i + train_size : i+1 + train_size]
    y_train, y_test = y[i : i + train_size], y[i + train_size : i+1 + train_size]
    ...
    print(f"XGBoost_Progress: {progress:.2f}%")


# Feature-Importances abrufen
feature_importances = model.feature_importances_

# Lags nach Wichtigkeit sortieren
sorted_indices = np.argsort(feature_importances)[::-1]  # Sortiert von wichtig nach weniger wichtig

# Ausgabe der wichtigsten Lags
print("Wichtigste Lags (nach Bedeutung sortiert):")
for idx in sorted_indices:
    print(f"Lag_{idx+1}:\t Importance: {feature_importances[idx]:.4f}")
...
plt.plot(df_pred.index[:train_size], y[:train_size], label="Trainingsdaten", color="black", linewidth=1)

# Testdaten (Expected) plotten
plt.plot(df_pred.index[train_size:], actual, label="Expected (Test)", color="0.75", linewidth=1)

# Vorhersagen plotten
plt.plot(df_pred.index[train_size:], predictions, label="Predicted", color="red", linewidth=1)




# Achsenlimits setzen (Startdatum = 2016-04-01)
plt.legend()
plt.title("Trainingsdaten, Testdaten und Vorhersagen")
plt.xlabel("Datum")
plt.ylabel("Calls")
#plt.grid()
